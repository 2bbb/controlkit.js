...
function ValuePlotter(parent,object,value,params) {
    ...
    params.normalize  = params.normalize  || false;
...
ValuePlotter.prototype._drawCurve = function () {
    ...
    if(this._normalize) {
        points[1] = buffer0[0];
        buffer0[bufferLength - 1] = value;

        var min = buffer0[0],
            max = buffer0[0];
        for(var m = 1; m < bufferLength; m++) {
            min = Math.min(buffer0[m], min);
            max = Math.max(buffer0[m], max);
        }
        unit *= 2;
        var interval = max - min,
            v = interval ? (1.0 - (points[1] - min) / interval) * unit : 0.0;
        pathCmd += this._pathCmdMoveTo(points[0], v);
        ...
        while (++i < bufferLength) {
            ...
            tmp = buffer0[i];
            points[index + 1] = buffer0[i - 1] = tmp;

            v = interval ? (1.0 - (points[index + 1] - min) / interval) * unit : 0.0;
            pathCmd += this._pathCmdLineTo(points[index], v);
        }
    } else {
        points[1] = buffer0[0];
